...
public class OverseaBaseOffer
{
	...
	public static void updateGpOfferToDb2(Map<String,String> blackMap){
		...
		StringBuffer pross=new StringBuffer();
		String pros=null;
		String pros_cahe=",";
		...
		for (int i = 0; i < proList.size(); i++) {
			...
			if(proList.get(i)!=null){
				String prov = proList.get(i).getProvider().toString();
				pross.append(prov+",");
				pros= pross.toString();
			}
		}
...
//		pros= pros+s;
		...
		logger.info("offerlist2:"+offer_list.size());
		...
		try {
			...
			logger.info("offerlist.remove(key)="+countk);
...
//-----------------------------------------------------------------------	
		...
		else
		{
			...
			logger.info("dbList list:"+dbList.size());
			// 初始化,
