    ...
    public int insertTem(final OverseaAd_Tem item)
    {
        ...
        sb
                .append(" (`type`,`provider`,`name`, `pkg`, `apk`, `country`,`ecountry`,"
                        + "`mainicon`, `visit`, `click`, `cap`,`object_cap`, `price`, `weight`,`preweight`,`pullratio`,`sinstall`,`status`, `offerid`,`is_onlypush`,`push_rate`,`is_exchange`,`createdate`,`updatedate`) VALUES (?,?,  ?,?, ?, ?, ?, ?, ?,?, ?, ?,?,?,?, ?, ?,?, ?,?,?,?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)");
        ...
        PreparedStatementCreator psc = new PreparedStatementCreator()
        {
                    ...
                    throws SQLException
            {
                ...
                if (item.getPush_rate()!= null)
                {
                	ps.setInt(i++, item.getPush_rate());
                }
                else
                {
                	 ps.setNull(i++, Types.NULL);
...
//        }
    ...
    public void updateStatus_Tem(final List<OverseaAd_Tem> item1)
    {
        for (final OverseaAd_Tem item : item1)
        {
            ...
            sb.append(table());
            sb
                    .append(" SET `click`=?,`status`=0 ,`updatedate`=CURRENT_TIMESTAMP  WHERE `provider`=? and `pkg`=? and `country`=? and `offerid`=? and is_manual=1");
    ...
    public void update2track(final OverseaAd item){
        ...
        sb
                .append(" SET `type`=?,`provider`=?, `name`=?, `pkg`=?, `apk`=?,"
                        + "`country`=?,`ecountry`=?,"
                        + "`mainicon` = ?, `visit`=?, `click`=?, `cap`=?,`object_cap`=?, "
                        + "`price`=?,`preweight`=?,`weight`=?, `status`=?,`sinstall`=?, `pullratio`=?,`offerid`=? WHERE `id`=?");
        PreparedStatementSetter psc = new PreparedStatementSetter()
        {
            public void setValues(PreparedStatement ps) throws SQLException
            {
...
//    }
    ...
    public void updateCom(final List<OverseaAd_Tem> item1)
    {
        ...
        for (final OverseaAd_Tem item : item1)
        {
    		...
    		sb.append("UPDATE ");
    		sb.append(table());
    		sb.append(" SET `ecountry`=?,`click`=?,`price`=?, `object_cap`=? ,`updatedate`=CURRENT_TIMESTAMP " +
